#include "stdafx.h"
#include "../headers/asset_holder.h"
...
void asset_holder::add_sound_buffer(int id, const std::string & filepath)
{
	auto temp_soundbuffer = std::make_unique<sf::SoundBuffer>();
	if (temp_soundbuffer->loadFromFile(filepath)) {
		m_soundbuffers[id] = std::move(temp_soundbuffer);
	}
	else {
...
void asset_holder::play_sound(int id,float volume)
{
	assert(m_soundbuffers.find(id) != m_soundbuffers.end());
	m_sounds.emplace_back(sf::Sound::Sound(*(m_soundbuffers[ id].get())));
	m_sounds.back().setVolume(volume);
	m_sounds.back().play();
}

 sf::Texture& asset_holder::get_texture( int id) 
{
 ...
 sf::Font& asset_holder::get_font(int id) 
{
 ...
 sf::SoundBuffer& asset_holder::get_soundbuffer(int id)
 {
	 assert(m_soundbuffers.find(id) != m_soundbuffers.end());
	 return *((m_soundbuffers[id]).get());
 }
